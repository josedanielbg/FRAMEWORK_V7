...
from .pipeline.live_prediction import (
    feature_profile,
    keras_experiment_artifact_paths,
    keras_experiment_options,
    load_keras_live_model,
    load_machine_learning_variables,
...
def render_experiments(data: ProjectData) -> None:
    ...
    with tab_interpretation:
        interpretation_experiments = _experiment_names(interpretation_summary)
        if not interpretation_experiments:
            render_missing_file(INTERPRETATION_DIR)
        else:
            default_interpretation = selected_experiment
            if default_interpretation not in interpretation_experiments:
                versioned_matches = [
                    experiment
                    for experiment in interpretation_experiments
                    if experiment.startswith(f"{selected_experiment}-")
                ]
                default_interpretation = (
                    versioned_matches[0] if versioned_matches else interpretation_experiments[0]
                )

            selected_interpretation = st.selectbox(
                "Experimento interpretado",
                interpretation_experiments,
                index=interpretation_experiments.index(default_interpretation),
                key="experiment_interpretation_selected",
            )
            summary = load_interpretation_summary(selected_interpretation)
            interpreted = interpretation_summary[
                interpretation_summary.get("Experimento", pd.Series(dtype=str)).astype(str)
                == selected_interpretation
            ]
            if summary.empty and interpreted.empty:
                render_missing_file(INTERPRETATION_DIR / selected_interpretation / "resumen_experimento.csv")
            else:
                ...
                sequence_metadata = load_sequence_metadata(selected_interpretation)
                if not sequence_metadata and "-" in selected_interpretation:
                    sequence_metadata = load_sequence_metadata(selected_interpretation.split("-")[0])
...
def _slider_step(minimum: float, maximum: float) -> float:
    ...
    if span == 0:
        return 1.0
    if span >= 1_000_000:
        return float(max(1.0, round(span / 100)))
    if span >= 100:
        return 1.0
    if span >= 10:
        return 0.1
...
def _live_input_key(experiment: str, variable: str) -> str:
    """Build a stable Streamlit key for one live prediction input."""

    return f"live_input_{experiment}_{variable}"


def _live_input_values(
    profile: pd.DataFrame,
    editable_profile: pd.DataFrame,
    experiment: str,
) -> dict[str, float]:
    """Read the latest live prediction values from Streamlit session state."""

    values = {}
    for row in editable_profile.itertuples(index=False):
        key = _live_input_key(experiment, row.Variable)
        values[row.Variable] = float(st.session_state.get(key, row.Mediana))
    for row in profile[profile["Variable"] == "Mes"].itertuples(index=False):
        values[row.Variable] = float(row.Mediana)
    return values


SEQUENCE_MODE_LABELS = {
    "full_window": "Escenario persistente",
    "last_step": "Choque puntual",
}

LIVE_PREDICTION_EXPERIMENT = "Exp04"


@st.cache_resource(show_spinner=False)
def _cached_keras_live_model(experiment: str):
    """Load Keras live artifacts once per Streamlit session."""

    return load_keras_live_model(experiment)
...
def render_live_prediction(data: ProjectData) -> None:
    ...
    experiments = keras_experiment_options()
    if experiments.empty:
        st.warning("No hay modelos .keras con variables aptas disponibles para prediccion live.")
        return

    experiments = experiments[experiments["Experimento"] == LIVE_PREDICTION_EXPERIMENT]
    if experiments.empty:
        available = ", ".join(keras_experiment_options()["Experimento"].tolist()) or "ninguno"
        st.warning(
            f"No se encontro {LIVE_PREDICTION_EXPERIMENT} entre los modelos aptos para "
            f"prediccion live. Modelos disponibles: {available}."
        )
        return

    selected_experiment = LIVE_PREDICTION_EXPERIMENT
    selected_row = experiments.loc[experiments["Experimento"] == selected_experiment].iloc[0]
    selected_target = str(selected_row["Variable objetivo"])
    selected_window = int(selected_row["Ventana"])
    selected_features = [
    ...
    if profile.empty:
        st.warning("No fue posible perfilar las variables aptas para prediccion.")
        return
    editable_profile = profile[profile["Variable"] != "Mes"].copy()

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Experimento", selected_experiment)
    ...
    if artifact_paths["scaler"] is not None:
        st.caption(f"Scaler: {rel(artifact_paths['scaler'])}")

    tab_manual, tab_explain, tab_batch = st.tabs(
        ["Simulador manual", "Variables aptas", "Carga CSV/Excel"]
    )

    with tab_manual:
        st.markdown("**Escenario con modelo Keras**")
        with st.form(key=f"live_prediction_form_{selected_experiment}"):
            sequence_mode = st.selectbox(
                "Alcance temporal del escenario",
                options=list(SEQUENCE_MODE_LABELS.keys()),
                format_func=lambda mode: SEQUENCE_MODE_LABELS[mode],
                help=(
                    "Escenario persistente aplica los valores a toda la ventana LSTM. "
                    "Choque puntual cambia solo el ultimo paso temporal."
                ),
                key=f"live_sequence_mode_{selected_experiment}",
            )
            for group in editable_profile["Capa"].drop_duplicates().tolist():
                group_profile = editable_profile[editable_profile["Capa"] == group]
                with st.expander(group, expanded=group in ["Clima y variabilidad", "Calidad y percepcion"]):
                    ...
                    for index, row in enumerate(group_profile.itertuples(index=False)):
                        ...
                        widget_key = _live_input_key(selected_experiment, row.Variable)
            ...
            submitted = st.form_submit_button("Generar prediccion")

        if not submitted:
            st.info("Ajusta las variables del escenario y presiona Generar prediccion.")
        else:
            input_values = _live_input_values(profile, editable_profile, selected_experiment)
            with st.spinner("Generando prediccion con el modelo Keras..."):
                try:
                    keras_model = _cached_keras_live_model(selected_experiment)
                    prediction = predict_with_keras_model(
                        keras_model,
                        input_values,
                        sequence_mode=sequence_mode,
                    )
                except Exception as error:  # pragma: no cover - Streamlit dependency guardrail
                ...
                scenario_rows = [
                    {
                        ...
                        "Valor usado": SEQUENCE_MODE_LABELS[sequence_mode],
                    }
                ]
                scenario_rows.extend(
                    {"Variable": variable, "Valor usado": value}
                    for variable, value in input_values.items()
                    if variable != "Mes"
                )
                scenario_input = pd.DataFrame(scenario_rows)
                with st.expander("Escenario enviado al modelo"):
                    st.dataframe(scenario_input, use_container_width=True, hide_index=True)

            with chart_col:
                if is_irca_classifier:
                    scenarios = pd.DataFrame(
                        [
                            {"Escenario": "Umbral de riesgo", "Prediccion": 0.5},
            ...
            if is_irca_classifier:
                gauge = go.Figure(
                    go.Indicator(
                        mode="gauge+number",
                        value=min(max(0.0, prediction), 1.0) * 100,
                        number={"suffix": "%"},
                        title={"text": "Probabilidad de riesgo IRCA"},
                        gauge={
                            "axis": {"range": [0, 100]},
                            "bar": {"color": "#2563EB"},
                            "steps": [
    ...
    with tab_explain:
        st.markdown("**Variables editables del simulador**")
        variables_ml = load_machine_learning_variables()
        if not variables_ml.empty and "Variable" in variables_ml.columns:
            selected_variables = variables_ml[
                variables_ml["Variable"].isin(editable_profile["Variable"])
            ]
        else:
            selected_variables = editable_profile
        st.dataframe(selected_variables, use_container_width=True, hide_index=True)
        ...
        fig = px.bar(
            ...
            y="Mediana",
            color="Capa",
            title="Mediana historica de variables de entrada",
        )
        fig.update_layout(height=430, margin=dict(l=10, r=10, t=55, b=10))
        st.plotly_chart(fig, use_container_width=True)

        st.markdown("**Artefactos de inferencia**")
        artifacts = pd.DataFrame(
            [
                {
                    "Artefacto": "Modelo Keras",
                    "Ruta": rel(artifact_paths["model"]) if artifact_paths["model"] else "-",
                },
                {
                    "Artefacto": "Scaler",
                    "Ruta": rel(artifact_paths["scaler"]) if artifact_paths["scaler"] else "-",
                },
                {
                    "Artefacto": "Scaler objetivo",
                    "Ruta": (
                        rel(artifact_paths["target_scaler"])
                        if artifact_paths.get("target_scaler")
                        else "-"
                    ),
                },
                {
                    "Artefacto": "Dataset transformado",
                    "Ruta": (
                        rel(artifact_paths["transformed_dataset"])
                        if artifact_paths["transformed_dataset"]
                        else "-"
                    ),
                },
            ]
    ...
    with tab_batch:
        ...
        uploaded_file = st.file_uploader(
            ...
            type=["csv", "xlsx"],
            key=f"live_upload_{selected_experiment}",
        )
        if uploaded_file is None:
            st.dataframe(
                editable_profile[["Variable", "Capa", "Mediana", "Min", "Max"]],
                use_container_width=True,
                hide_index=True,
            )
        else:
            ...
            for feature in selected_features:
                if feature not in scoring_df.columns:
                    default = float(profile.loc[profile["Variable"] == feature, "Mediana"].iloc[0])
                    scoring_df[feature] = default
            st.dataframe(scoring_df[selected_features].head(50), use_container_width=True, hide_index=True)
            if not st.button(
                "Generar predicciones del archivo",
                key=f"batch_predict_{selected_experiment}",
            ):
                st.info("Revisa la muestra de variables y presiona Generar predicciones del archivo.")
                return

            with st.spinner("Generando predicciones del archivo..."):
                keras_model = _cached_keras_live_model(selected_experiment)
                predictions = scoring_df[selected_features].apply(
                    lambda row: predict_with_keras_model(keras_model, row.to_dict()),
                    axis=1,
                )
            ...
            st.download_button(
                ...
                file_name=f"predicciones_live_{selected_experiment}_{selected_target}.csv",
                mime="text/csv",
                key=f"download_live_predictions_{selected_experiment}",
            )
...
def render_experiment_design(data: ProjectData) -> None:
    ...
    tab_map, tab_config, tab_diagnostic, tab_files = st.tabs(
        ["Mapa experimental", "Configuracion", "Diagnostico", "Archivos"]
    )
    ...
    with tab_diagnostic:
        modeling_summary = summarize_modeling_experiments()
        if not modeling_summary.empty and "Diagnostico" in modeling_summary.columns:
            diagnostic_summary = modeling_summary[modeling_summary["Diagnostico"] == True]
        else:
            diagnostic_summary = pd.DataFrame()
        diagnostic_experiments = _experiment_names(diagnostic_summary)
        if not diagnostic_experiments:
            render_missing_file(MODELING_DIR / "Diagnosticos")
        else:
            selected_diagnostic = st.selectbox(
                "Experimento",
                diagnostic_experiments,
                key="design_diagnostic_experiment",
            )
            diagnostic = load_model_diagnostic(selected_diagnostic)
            recommendations = load_model_recommendations(selected_diagnostic)
            if diagnostic.empty:
                render_missing_file(
                    MODELING_DIR
                    / "Diagnosticos"
                    / selected_diagnostic
                    / f"diagnostico_modelo_{selected_diagnostic}.csv"
                )
            else:
                st.markdown(f"**Resultado del modelo {selected_diagnostic}**")
                metric_values = diagnostic.copy()
                metric_values["Valor_Numerico"] = pd.to_numeric(metric_values["Valor"], errors="coerce")
                chart_values = metric_values.dropna(subset=["Valor_Numerico"])
                if not chart_values.empty:
                    fig = px.bar(
                        ...
                        y="Valor_Numerico",
                        color="Estado",
                        title=f"Metricas del modelo {selected_diagnostic}",
                        color_discrete_map={
                            "Excelente": "#2A9D8F",
                            "Aceptable": "#E9C46A",
                            "Baja": "#F4A261",
            ...
            images = sorted((MODELING_DIR / "Metricas" / selected_diagnostic).glob("*.png"))
            if images:
