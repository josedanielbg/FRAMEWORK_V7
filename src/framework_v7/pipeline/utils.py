...
def read_table(path: Path, **kwargs) -> pd.DataFrame:
    ...
    Centralizes notebook IO for CSV, Excel and parquet files. Missing files
    return an empty DataFrame so exploratory notebooks can continue and decide
    how to report missing artifacts.

    Args:
        path (Path): File path to read. Supported suffixes are ``.csv``,
            ``.xlsx``, ``.xls`` and ``.parquet``.
        **kwargs: Extra keyword arguments forwarded to the relevant pandas
            reader.
