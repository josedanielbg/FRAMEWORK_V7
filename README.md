...
- Simulador de prediccion live para `VolumenUtilDiarioMasa` con el
  experimento `Exp04`.
...
- `Experimentos`: resultados, model cards, metricas, predicciones e
  interpretacion.
- `Prediccion live`: simulador interactivo de `Exp04` y carga de CSV/Excel
  para inferencia exploratoria.
...
salida base es un indice normalizado del modelo. Para facilitar la lectura
operativa, la app agrega una calibracion min-max contra el rango historico de
`VolumenUtilDiarioMasa` y muestra un volumen aproximado en m3. Las evaluaciones
...
- Consultar variables aptas, rutas del modelo, scaler y dataset transformado.
- Cargar archivos CSV/Excel y descargar predicciones generadas.

Este simulador es una herramienta exploratoria para analisis de escenarios sobre
los artefactos entrenados del repositorio.
